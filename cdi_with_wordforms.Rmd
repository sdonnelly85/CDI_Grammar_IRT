...
As a quick sanity check, let's see which items have ordinal data
...
```{r  message=FALSE, warning=FALSE}
m1 <- mirt(Complex_short, 1, itemtype="graded")
```
...
This block of code will need to be fixed if I treat variables as ordinal, as the output of coef is now different. 
```{r}
...
```{r}
m2 <- mirt(Complex_short, 1, itemtype="graded")
...
```{r}
m2b <- mirt(Complex_short, 2, "graded")
```
...
```{r}
m3b <- mirt(Complex_short, 2, "graded")
...
```{r}
m1b <- mirt(Complex_short, 2, "graded")
```
...
Model fits a little better, but the guessing parameters were all above .9 so I don't think those are adding very much. 


Comapre with 1 factor 2PL model with semi-parametric latent variable specification. 
```{r}
m1_nonpar <- mirt(Complex_short, 1, itemtype="graded", dentype = "EH")
...
```{r}
m1_nonpar2 <- mirt(Complex_short, 1, itemtype="graded", dentype = "EHW")
...
I tried using the DifR package, but I was getting error messages that I didn't understand. Moreover, this package uses `ltm` to estimate the model, which gives me very different results from `mirt`. I toyed with the control parameter settings but couldn't get them to produce the same results. 
...
dif <- lordif(Complex_short2, 
              ...
              alpha = .01)
```
Only 2 items out of 41 have dif by gender. That's really interesteined. 
