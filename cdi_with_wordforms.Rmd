...
```
```{r}
m3c <- mirt(Complex_short, 1, "3PLu")
summary(m3c, "oblimin", suppress=.20)
```
...
Three parameter model with inverse guessing parameter. 
```{r}
m1c <- mirt(Complex_short, 1, "3PLu")
```
```{r}
anova(m1c, m1)
```
...
```{r}
m1_nonpar2 <- mirt(Complex_short, 1, itemtype="2PL", dentype = "EHW")
...
These semi-parametric alternatives look pretty similar. 
...
So I'll use the stuff in MIRT
```{r}
mg <- multipleGroup(Complex_short, 1, group = Complex_with_predictors$sex)
```

```{r}
DIF(mg, c('a1', 'd'), p.adjust="bonferroni", plotdiff=TRUE)
```
