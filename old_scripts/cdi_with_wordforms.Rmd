...
This is an old script. I was exploring whether we could include the the other grammatical items to the IRT model of syntactic complexity, so as to get information about some of the participants whose parents reported that they weren't producing any of the relatively complex items on the CDI. I tried considering the various word form subscales, which ask about various forms of regular and irregular morphological marking. 
...
"61", "62", "63", "64", "65", "66", "67") # create labels for corPlot because the definitions are long and take up a ton of space
```
...
plot(pc)
```
Looks like there's a big clump of vectors in one area but then way more pointing out in random directions, which I think will create some fit issues. 
...
Let's look at the joint distribution of the two latent variables. 

```{r}
Complex_short %>%
  mutate(
    Raw = rowSums(.[,1:67]), 
    Theta1 = fscores(m2b)[,1], 
    Theta2 = fscores(m2b)[,2]
  ) %>%
...
Complex_short %>%
  mutate(
    Raw = rowSums(.[,1:87]), 
    Theta1 = fscores(m3b)[,1], 
    Theta2 = fscores(m3b)[,2]
  ) %>%
  dplyr::select(Raw, Theta1, Theta2) %>%
  ggpairs()
...
Note on 18/11/2021. This section is mostly irrelevant. We decided to only include complexity scale items for participants whose parents reported that they were combining words. Therefore, we didn't include these other variables. 
