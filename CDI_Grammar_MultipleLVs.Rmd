...
Download relevant data sets (Item = Descriptions of each Item on the Scale, Inst = item-level scores for each participant, Admin = Participant-Level data). Outcome variable needs to be re-coded from "complex" and "simple" to 1 and 0. Remove observations from longitudinal data sets. Save combine data because I'm going to remove participants who are not yet combining words. 
```{r}
Complex <- Admin %>%
...
Create a variable that describes each item. 
```{r}
...
ggsave("plots/DiscDiffGauss.png", last_plot(), dpi=300, height =3, width = 4, unit="in")
```
...
p2 <- true_raw/(true_score + raw_score) 
ggsave("plots/PredictedRawGauss.png", last_plot(), dpi=300, height =3, width = 4, unit="in")
```
There are still some 0s but this is considerably fewer than the dataset with all participants included. 

```{r}
Theta <- tibble(Theta = fscores(m1))
Theta2 <- matrix(seq(-4,4,.01))
m1_inf <- testinfo(m1, Theta2)
m1_inf <- tibble(
  Theta=Theta2, 
  Info = m1_inf
)

hist(Theta, data=Theta)
par(new=TRUE)
plot(x=m1_inf$Theta, y=m1_inf$Info, type="l", pch = 15, lwd=2, col = 2, axes = FALSE, xlab = "", ylab = "") 
axis(side = 4, at = pretty(range(m1_inf$Info))) 
abline(h = 10)
```
...
```{r}
Theta <- tibble(Theta = fscores(m1_EH))
Theta2 <- matrix(seq(-4,4,.01))
m1_EH_inf <- testinfo(m1_EH, Theta2)
m1_EH_inf <- tibble(
  Theta=Theta2, 
  Info = m1_EH_inf 
)
...
hist(Theta, data=Theta)
par(new=TRUE)
plot(x=m1_EH_inf$Theta, y=m1_EH_inf$Info, type="l", pch = 15, lwd=2, col = 3, axes = FALSE, xlab = "", ylab = "") 
axis(side = 4, at = pretty(range(m1_EH_inf$Info)))
abline(h = 10)
```
...
p3 <- Complex_short_grammatical %>%
  ...
  ggplot(aes(x=theta, fill=model)) + geom_histogram(alpha = 0.5) + theme_minimal()

ggsave("plots/LVDist.png", last_plot(), heigh=3, width=4, dpi=300, unit="in")
```
